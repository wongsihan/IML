...
from common_nomayavi import App, Plotter, tensor
...
class Shift2D(nn.Module):
    def __init__(self, points, fixed_points, fixed_h=False):#points 100 fixed_points 40
        super().__init__()
        n_free = len(points[0])#100
        n_fixed = len(fixed_points[0])#40
        self.n = n = n_free + n_fixed#140
        if n_fixed:
            dsize = np.ptp(np.hstack((points[0], fixed_points[0])))#1.0
        else:
            dsize = np.ptp(points[0])
        dx = dsize/np.sqrt(n)#0.0845
        self.dx = dx
        ...
        if fixed_h:
            ...
            self.h = nn.Parameter(dx*torch.ones(n))#140
    ...
    def forward(self, x, y):#(250,1) (250,1)
        xc, yc = self.centers()#140 140
        fac = 1.0/self.h#140 这个self.h就是基函数的分母hi
        xh = (x - xc)*fac#(250,140)已经减去Xi并除以hi
        yh = (y - yc)*fac#(250,140)已经减去Xi并除以hi
        return xh, yh
...
class SPINN2D(nn.Module):
    ...
    def __init__(self, pde, activation, fixed_h=False, use_pu=False):
        ...
        self.layer1 = Shift2D(pde.nodes(), pde.fixed_nodes(),#nodes 100内部 fixed_nodes 40边界
                              fixed_h=fixed_h)
        n = self.layer1.n
        self.layer2 = nn.Linear(n, pde.n_vars(), bias=not use_pu)#140 3
        self.layer2.weight.data.fill_(0.0)
        if not self.use_pu:
            self.layer2.bias.data.fill_(0.0)

    def forward(self, x, y):
        x = x.unsqueeze(1)#(250,1)
        y = y.unsqueeze(1)#(250,1)
        xh, yh = self.layer1(x, y)#(250,140) (250,140)已经减去Xi并除以hi
        z = self.activation(xh, yh)#(250,140)
        if self.use_pu:
