...
class CavityPDE(RegularPDE):
    def __init__(self, n_nodes, ns, nb=None, nbs=None, sample_frac=1.0):#生成100+40个固定点和2500个采样点
        ...
        rm = 0.045
        p = 5
        self.lengthX = alpha * rm * Pi / p
        self.weight_bound = 1
        self.weight_inner = 1
        self.losses = torch.zeros(11)

        # 永磁体
        self.thickness_mag = 0.010
        dxb2 = 1.00 / (1000*n)
        self.fitness = 0.00
        xl, xr = self.fitness, self.lengthX - self.fitness
        ...
        if nb == 0:
            ...
            _x = np.linspace(self.fitness, self.lengthX - self.fitness, nb)
            _o = np.ones_like(_x)
            _l = np.linspace(self.fitness, lengthY4 - self.fitness, nb)
            x = np.hstack((_x, _x, _x, _x, _x*0.0, _o*self.lengthX))
        ...
        _x = np.linspace(self.fitness, self.lengthX - self.fitness, nbs)
        _o = np.ones_like(_x)
        ...
        self.six = six = (tg(_o*self.lengthX), tg(_l))
        self.b_samples = (
    ...
    def mtl_uw(self, nn,loss):
        loss_re = (loss / (2 * nn.loss_scale.exp()) + nn.loss_scale / 2).sum()
        return loss_re
    ...
    def interior_loss(self, nn):
        ...
        Ady_cor_copper = xs[self.label + self.label2:].reshape(10,10)
        Adz_cor_copper = ys[self.label + self.label2:].reshape(10,10)
        Ady_copper = Ady[self.label + self.label2:].reshape(10,10)
        ...
        # #mz = (4 * Br / (Pi * u0)) * (torch.sin(alpha * Pi / 2)) * (torch.cos(tensor(-1*beta*xvar+(Pi/2))))
        # mz = (4 * Br / (sp.pi * u0)) * (sp.sin(alpha * sp.pi / 2)) * (sp.cos(-1 * beta * xvar + (sp.pi/2)))
        # #mzy = ((4 * Br) /(Pi * u0)) * (torch.sin(alpha * Pi / 2)) * beta * (torch.sin((-1)*beta * (xsA1-((rm*Pi/5)/2))))
        # for n in range(2,11):
        #     mz = mz + ((4 * Br) / ((2*n-1)*sp.pi * u0)) * (sp.sin((2*n-1)*alpha * sp.pi / 2)) * (sp.cos((-1)*(2*n-1)*beta * xvar + (2*n-1)*(sp.pi/2)))
        # mzy = sp.diff(mz, xvar)
        # #mzy = tensor(mzy)
        # f = sp.lambdify(xvar, mzy, "numpy")
        # xsA1 = xsA1.detach().numpy()
        # mzy_num = f(xsA1)
        mz = (4 * Br / (Pi * u0)) * (torch.sin(alpha * Pi / 2)) * (torch.cos(tensor(-1 * beta * xsA1 + (Pi / 2))))
        for n in range(2, 3):
          mz = mz + ((4 * Br) / ((2*n-1)*Pi * u0)) * (torch.sin((2*n-1)*alpha * Pi / 2)) * (torch.cos((-1)*(2*n-1)*beta * xsA1 + (2*n-1)*(Pi/2)))

        mzy_num, _ = self._compute_gradient(mz, xsA1, xsA1)
        ...
        self.losses[0] = loss_one.item()
        self.losses[1] = loss_two.item()
        # loss_one_re = self.mtl_uw(nn,loss_one)
        # loss_two_re = self.mtl_uw(nn,loss_two)
#########
...
#         exact_eddy_current = -sigma * omega * rm * exact_A2dy  # [46,46]
#         J2 = exact_eddy_current * exact_eddy_current
#         dy = y3
#         dx = x3
#         exact_torque = integrate.simps(J2.T.cpu().detach().numpy(), dy.T.cpu().detach().numpy(), axis=0)
#         exact_torque = (integrate.simps(exact_torque, dx[:,0].cpu().detach().numpy()) * L * 10) / (sigma * omega)
#         #print("exact torque is：",exact_torque)
#
...
#         dy = Adz_cor_copper.T
#         dx = Ady_cor_copper[:, 0]
#         torque = integrate.simps(J2.cpu().detach().numpy().T, dy.cpu().detach().numpy(), axis=0)
#         torque = (integrate.simps(torque, dx.cpu().detach().numpy()) * L * 10) / (sigma * omega)
#         loss_three = ((exact_torque - torque) ** 2)
 ########################
        ...
        return loss, loss_one, loss_two# [2]41.8,2.7 [10001] 8.9, 0.16, 0.3



    def conform_exact(self,Ady_pre,Adz_pre,y,z,pattern,boundary):
        weight = self.weight_bound
        alpha = 1
        Pi = np.pi
        br = 1.25
        rm = 0.045
    ...
    def boundary_loss(self, nn):
        ...
        loss_three = self.conform_exact(Ady_one, Adz_one, xb, yb, 2, 1)
        self.losses[2] = loss_three.item()

        bc_loss += self.conform_exact(Ady_two, Adz_two, xb, yb, 2, 2)
        loss_four = self.conform_exact(Ady_two, Adz_two, xb, yb, 2, 2)
        self.losses[3] = loss_four.item()

        bc_loss += ((A1dy2 + A1dz2) ** 2).sum()#0.005
        loss_five = ((A1dy2 + A1dz2) ** 2).sum()
        self.losses[4] = loss_five.item()

        bc_loss += ((A2dy2 + A2dz2) ** 2).sum()#0.0356
        loss_six = ((A2dy2 + A2dz2) ** 2).sum()
        self.losses[5] = loss_six.item()
        ...
        # mzy = ((4 * Br) / (Pi * u0)) * (torch.sin(alpha * Pi / 2)) * beta * (
        #     torch.sin((-1) * beta * (xbA3 - ((rm * Pi / 5) / 2))))
        # for n in range(2, 101):
        #     mzy = mzy + ((4 * Br) / ((2 * n - 1) * Pi * u0)) * (torch.sin((2 * n - 1) * alpha * Pi / 2)) * beta *(2*n-1)* (
        #         torch.sin((-1) * (2 * n - 1) * beta * (xbA3 - ((rm * Pi / 5) / 2))))
        # #bc_loss += self.conform_exact(Ady_three, Adz_three, xb, yb, 2, 3)#285   [10001]8.266
        # # bc_loss += (((A3dy2 + A3dz2 + u0 * mzy) ** 2)*bc_weight).sum()#8.5e-17 [10001]0.914
        mz = (4 * Br / (Pi * u0)) * (torch.sin(alpha * Pi / 2)) * (torch.cos(tensor(-1 * beta * xbA3 + (Pi / 2))))
        for n in range(2, 3):
            mz = mz + ((4 * Br) / ((2 * n - 1) * torch.pi * u0)) * (torch.sin((2 * n - 1) * alpha * torch.pi / 2)) * (
                torch.cos((-1) * (2 * n - 1) * beta * xbA3 + (2 * n - 1) * (torch.pi / 2)))

        mzy_num, _ = self._compute_gradient(mz, xbA3, xbA3)
        bc_loss += self.conform_exact(Ady_three, Adz_three, xb, yb, 1, 3)#285  [10001]8.266
        loss_seven = self.conform_exact(Ady_three, Adz_three, xb, yb, 1, 3)
        self.losses[6] = loss_seven.item()

        bc_loss += (((A3dy2 + A3dz2 + u0 * mzy_num) ** 2) ).sum()
        loss_eight = (((A3dy2 + A3dz2 + u0 * mzy_num) ** 2) ).sum()
        self.losses[7] = loss_eight.item()

        bc_loss += ((A3dy2 + A3dz2) ** 2).sum()#0.0046
        loss_nine = ((A3dy2 + A3dz2) ** 2).sum()
        self.losses[8] = loss_nine.item()
        # 4边遵循 A1
        ...
        # mzy = ((4 * Br) / (Pi * u0)) * (torch.sin(alpha * Pi / 2)) * beta * (torch.sin((-1) * beta * xbA4))
        # for n in range(2, 11):
        #     mzy = mzy + ((4 * Br) / (n * Pi * u0)) * (torch.sin(n * alpha * Pi / 2)) * beta * (
        #         torch.sin((-1) * beta * xbA4))
        mz = (4 * Br / (Pi * u0)) * (torch.sin(alpha * Pi / 2)) * (torch.cos(tensor(-1 * beta * xbA4 + (Pi / 2))))
        for n in range(2, 3):
            mz = mz + ((4 * Br) / ((2 * n - 1) * torch.pi * u0)) * (torch.sin((2 * n - 1) * alpha * torch.pi / 2)) * (
                torch.cos((-1) * (2 * n - 1) * beta * xbA4 + (2 * n - 1) * (torch.pi / 2)))

        mzy_num, _ = self._compute_gradient(mz, xbA4, xbA4)
        bc_loss += self.conform_exact(Ady_four, Adz_four, xb, yb, 1, 4)#326 [10001]2.4058
        loss_ten = self.conform_exact(Ady_four, Adz_four, xb, yb, 1, 4)
        self.losses[9] = loss_ten.item()


        bc_loss += ((A4dy2 + A4dz2 + u0 * mzy_num) ** 2).sum()#4.2e-19 [10001]2.93
        loss_eleven = ((A4dy2 + A4dz2 + u0 * mzy_num) ** 2).sum()
        self.losses[10] = loss_eleven.item()
        # 5 6边赋常数
        # Ady_five = Ady_five * 0
        # Adz_five = Adz_five * 0
        # Ady_six = Ady_six * 0
        # Adz_six = Adz_six * 0
        # bc_loss += self.conform_exact_boundary(Ady_five, Adz_five, xb, yb, 5)#39 [10001] 0.1341
        # bc_loss += self.conform_exact_boundary(Ady_six, Adz_six, xb, yb, 6)#39 [10001] 0.1428

        return bc_loss, loss_three,loss_four, loss_five,loss_six,loss_seven,loss_eight,loss_nine,loss_ten,loss_eleven
    ...
    def plot_points(self):
        ...
        x, y = np.mgrid[self.fitness:length:n * 1j, self.fitness:thickness_sum-self.fitness:n * 1j]
        x = x
        y = y
        x_copper, y_copper = np.mgrid[self.fitness:length:n * 1j, thickness_mag_air:thickness_sum:n * 1j]
...
if __name__ == '__main__':
    ...
    app.run(nodes=30, samples=100, sample_frac=1, lr=5e-5, n_train=50000)#100个内部样本点，40个边界样本，2500总抽样
