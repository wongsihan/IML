...
class CavityPDE(RegularPDE):
    def __init__(self, n_nodes, ns, nb=None, nbs=None, sample_frac=1.0):#生成100+40个固定点和2500个采样点
        ...
        # 气隙 900
        lengthY = 0.03
        yb, yt = 0.10 + dxb2, 0.10 + lengthY - dxb2
        sly = slice(yb, yt, n * 1j * 0.3)
        x2, y2 = np.mgrid[slx, sly]

        # 铜 2500
        lengthY = 0.05
        yb, yt = 0.13 + dxb2, 0.13 + lengthY - dxb2
        ...
        if nb == 0:
            ...
            y = np.hstack((_o*0.0, _o*0.1, _o*0.13, _o*0.18, _l, _l))
        ...
        # 气隙 900
        lengthY = 0.03
        yb, yt = 0.1 + dxb2, 0.1 + lengthY - dxb2
        ...
        self.one = one = (tg(_x), tg(_o * 0.18))
        self.two = two = (tg(_x), tg(_o * 0.13))
        self.three = three = (tg(_x), tg(_o * 0.1))
    ...
    def interior_loss(self, nn):
        ...
        rm = tensor(0.45, requires_grad=True)
    ...
    def plot_points(self):
        n = min(self.ns * 2, 200)
        length = 0.081 * np.pi  # ns 50 n 100
        x, y = np.mgrid[0:length:n * 1j, 0:0.18:n * 1j]
        x_copper, y_copper = np.mgrid[0:length:n * 1j/2, 0.13:0.18:n * 1j/2]
...
class CavityPlotter(Plotter2D):
    ...
    def plot_solution(self):
        ...
        #A2dy = Ady.ravel()[self.label:]
        vmag = Ady
        eddy_current = 57000000*5*np.pi*0.45*Ady_copper#[46,46]
        torque = integrate.simps(eddy_current*eddy_current, yn_copper.T, axis = 0)
        torque = (integrate.simps(torque, xn_copper[:, 0]) * 10 * 0.3) / (57000000 * 5*np.pi)
        # torque = (eddy_current *eddy_current).sum()
        # torque = torque / len(eddy_current[0]**2)
        # torque =  (torque * 10 * 0.3)/ (57 * 5*np.pi)

        #torque = (torque.sum())/len()
        pde = self.pde
        if self.plt1 is None:
            ...
            self.plt1.mlab_source.trait_set(scalars=vmag)
        print("Plot Torque is :", torque)
