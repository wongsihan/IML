...
from common_nomayavi import PDE, tensor
from spinn2d_nomayavi import Plotter2D, SPINN2D, App2D
...
class RegularPDE(PDE):
    ...
    def __init__(self, n_nodes, ns, nb=None, nbs=None, sample_frac=1.0):
        ...
        # Interior nodes内部基函数数量的确定
        n = round(np.sqrt(n_nodes) + 0.49)#采样点开根号 50
        self.n = n
        dxb2 = 0.5/(n + 1)
        xl, xr = dxb2, 1.0 - dxb2#左右采样边界
        sl = slice(xl, xr, n*1j)
        x, y = np.mgrid[sl, sl]
        self.i_nodes = (x.ravel(), y.ravel())#100 100

        # Fixed nodes边界基函数数量的确定
        ...
        self.rng_interior = np.arange(self.n_interior)#2500
        ...
        self.b_samples = (xb, yb)#返回的是xl xr之外 定义域内的
    ...
    def interior(self):
        if abs(self.sample_frac - 1.0) < 1e-3:
            return self.p_samples,self.p_samples_x1,self.p_samples_x2,self.p_samples_x3,self.p_samples_y1,self.p_samples_y2,self.p_samples_y3
        else:
