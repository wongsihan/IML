...
class CavityPDE(RegularPDE):
    def __init__(self, n_nodes, ns, nb=None, nbs=None, sample_frac=1.0):#生成100+40个固定点和2500个采样点
        ...
        if nb == 0:
            ...
            x = np.hstack((_x, _x, _x, _x, _x*0.0, _o*56.54))*0.1
            y = np.hstack((_o*0.0, _o*10.0, _o*13.0, _o*18.0, _l, _l))*0.1
        ...
        _x = np.linspace(dxb2, lengthX - dxb2, nbs)*0.1
        _o = np.ones_like(_x)*0.1
        _l = np.linspace(dxb2, lengthY4 - dxb2, nbs)*0.1
        # x = np.hstack((_x, _o, _x, 0.0*_o))#堆叠参数的个数决定多少个
        # y = np.hstack((_o*0.0, _x, _o, _x))
    ...
    def interior_loss(self, nn):
        ...
        rm = tensor(4.5, requires_grad=True)
    ...
    def plot_points(self):
        ...
        x, y = np.mgrid[0:56.54:n * 1j, 0:18:n * 1j]*0.1
        x_copper, y_copper = np.mgrid[0:56.54:n * 1j, 13:18:n * 1j]*0.1
        # self.label2 =
...
class CavityPlotter(Plotter2D):
    ...
    def plot_solution(self):
        ...
        vmag = np.sqrt(Ady * Ady + Adz * Adz)
        eddy_current = 57000000 * 5*np.pi * 4.5 * Ady_copper  # [46,46]
        torque = integrate.simps(eddy_current * eddy_current, yn_copper.T,axis=0)
        torque = (integrate.simps(torque, xn_copper[:, 0]) * 3 * 10) / (57000000 * 5*np.pi)
        pde = self.pde
