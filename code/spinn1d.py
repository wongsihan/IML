...
class Plotter1D(Plotter):
    ...
    # Plotting methods
    def get_plot_data(self):#从函数文件中选取25个点
    ...
    def plot(self):
        ...
        err = self.plot_solution()#这里的err是25个采样点与解析解之间的三种err
        self.plot_weights()#在x轴上画center点
        if first:
            plt.legend()
            plt.pause(0.001)
        else:
...
class Shift(nn.Module):
    def __init__(self, points, fixed_points, fixed_h=False):
        ...
        dx = (xmax - xmin)/n
        self.center = nn.Parameter(tensor(points))#这使得五个center都可以被训练了
...
class SPINN1D(nn.Module):
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        y = self.layer1(x)#[25,7] 选定了7个点，又抽样了25个点，这里返回了|x-Xi|/hi
        y = self.activation(y)
        if self.use_pu:
